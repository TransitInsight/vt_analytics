...
def test_get_count_by_fc():
    ...
    assert  df_all['FaultCount'].count() > df_one['FaultCount'].count() 
...
def test_get_count_by_daterange():
    ...
    assert  df1['FaultCount'].count() < df2['FaultCount'].count()
...
def test_get_fc():
    ...
    assert df['faultName'].count() == 15
    assert df['faultCode'].count() == 15
...
def test_get_fc_location():
    ...
    assert df['LocationName'].count() > 1000
    assert len(df['faultName'].unique()) == 15

    df1 = vobcDA.get_count_trend(-1, '2014-01-01T00:00:00', '2015-04-25T00:13:26.017995', 2)
    assert df['LocationName'].count() > df1['LoggedDate'].count()
